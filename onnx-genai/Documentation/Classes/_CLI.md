...
| controller | 4D.Class | -> | Controller class to attach (default: `cs.ONNX._CLI_Controller`) |

## Description

`_CLI` resolves the platform-specific path to a bundled executable and attaches a controller that manages the underlying `4D.SystemWorker`. It is extended by [`_ONNX`](_ONNX.md) and should not be instantiated directly.
...
| controller | cs.ONNX._CLI_Controller | Attached controller instance |
...
- [`_ONNX`](_ONNX.md) — extends `_CLI` to wrap `onnx-genai`
