...
> _CLI_Controller.new (CLI : cs.ONNX._CLI)
...
| instance | cs.ONNX._CLI | The owning `_CLI` instance |
...
#### execute (command, message, context) → cs.ONNX._CLI_Controller
...
| Result | cs.ONNX._CLI_Controller | <- | `This` — enables chaining |
...
- [`_ONNX_Controller`](_ONNX_Controller.md) — extends `_CLI_Controller` with `onTerminate` forwarding
